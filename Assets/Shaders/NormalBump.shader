Shader "Custom/NormalBump" {
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
            ...
            float shift = noise(uv.x * _TextureShiftGranularity + _Time * 10.3, 
                                uv.y * _TextureShiftGranularity + _Time * 11.4, 
                                _TextureShiftGranularity + _Time * 10.7) * _NShift;
            float uMod = IN.uv_MainTex.x + shift % 1;
            float vMod = IN.uv_MainTex.y + (shift*2) % 1;
            fixed4 c2 = tex2D(_MainTex, float2(uMod, vMod)) * _Color;
			o.Albedo = (c.rgb + c2.rgb) * 0.5f;
